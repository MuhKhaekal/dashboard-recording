...
async function getDashboardData() {
  ...
  const latestDate = data[data.length - 1].recorded_at;
  const latestMetrics = data.filter((d) => d.recorded_at === latestDate);
  ...
  return (
    <main className="relative bg-slate-50 dark:bg-[#020617] min-h-screen lg:h-screen w-full text-slate-900 dark:text-white flex flex-col lg:flex-row p-4 md:p-6 gap-6 transition-colors duration-500 overflow-y-auto lg:overflow-hidden">
      ...
      <div className="relative z-10 w-full lg:w-[420px] flex flex-col gap-4">
        ...
        <div className="bg-white/70 dark:bg-slate-900/40 backdrop-blur-md border border-slate-200 dark:border-slate-800 p-3 rounded-2xl flex items-center gap-3 overflow-hidden">
          <marquee className="text-[10px] font-mono text-slate-500 dark:text-slate-400">
            System Sync Complete ... Database Latency: 24ms ... Kiln Overhaul Phase 2 Initiated ...
          </marquee>
        </div>
